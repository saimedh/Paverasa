...
>(({ text, index, children, className, ...props }, ref) => {
  ...
  // Flatten to get total character count for staggered delay, but keep structure for rendering
  let charCount = 0;
      ...
      {words.map((word, wordIndex) => (
        <span key={`word-${wordIndex}`} className="inline-block whitespace-pre">
          {word.split("").map((char, charIndexInWord) => {
            const currentGlobalIndex = charCount++;
            return (
              <span
              ...
              >
                <MotionConfig
                  transition={{
                    delay: currentGlobalIndex * 0.025,
                    duration: 0.3,
